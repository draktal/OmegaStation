...
ent-DieObjective = Умереть славной смертью.
    .desc = Умрите.
ent-HijackShuttleObjective = Устроить геноцид на эвакуационном шаттле (ХАЙДЖЕК)
    .desc = Не дайте экипажу попасть на центральное командование любым способом. Вы можете как запросить помощь второго агента, так и по связи (АХЕЛП) запросить дополнительные ТК. Удачи, агент.
